  ...
  const VerifyEmail = async () => {
    try {
      const res = await axios.get(
        `http://localhost:8080/verifyEmail/${token}`
      );
      const data = res.data;
      console.log(data);
  ...
  return (
    <div className="h-[100dvh] w-[100dvw] flex">
      <div className="max-w-sm flex flex-col rounded overflow-hidden m-auto shadow-lg ">
        <img className="w-full" src={logo} alt="Verified User" />
        <div className="px-6 py-4">
          ...
          <p className="text-gray-700 text-base">
            Now you can login and enjoy the courses, Now you can login and enjoy
            the courses
          </p>
        </div>
